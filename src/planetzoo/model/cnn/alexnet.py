...
class AlexNet(nn.Module):
    def __init__(
        ...
        """
        AlexNet with configurable convolutional kernel sizes, strides, channels, and linear layer sizes.
        ...
        self.features = nn.Sequential(
            # Conv1
            nn.Conv2d(in_channels[0], hidden_channels[0], kernel_size=kernel_sizes[0], 
                     stride=(hstrides[0], wstrides[0]), padding=2),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=3, stride=2),
            
            # Conv2
            nn.Conv2d(in_channels[1], hidden_channels[1], kernel_size=kernel_sizes[1], 
                     stride=(hstrides[1], wstrides[1]), padding=2),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=3, stride=2),
            
            # Conv3
            nn.Conv2d(in_channels[2], hidden_channels[2], kernel_size=kernel_sizes[2], 
                     stride=(hstrides[2], wstrides[2]), padding=1),
            nn.ReLU(inplace=True),
            
            # Conv4
            nn.Conv2d(in_channels[3], hidden_channels[3], kernel_size=kernel_sizes[3], 
                     stride=(hstrides[3], wstrides[3]), padding=1),
            nn.ReLU(inplace=True),
            
            # Conv5
            nn.Conv2d(in_channels[4], hidden_channels[4], kernel_size=kernel_sizes[4], 
                     stride=(hstrides[4], wstrides[4]), padding=1),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=3, stride=2),
        )
        ...
        # Classifier layers - use final conv layer output channels and configurable linear sizes
        final_channels = hidden_channels[-1]
        input_features = final_channels * 6 * 6
        
        self.classifier = nn.Sequential(
            nn.Dropout(dropout),
            nn.Linear(input_features, linear_sizes[0]),
            nn.ReLU(inplace=True),
            nn.Dropout(dropout),
            nn.Linear(linear_sizes[0], linear_sizes[1]),
            nn.ReLU(inplace=True),
            nn.Linear(linear_sizes[1], num_classes),
        )
